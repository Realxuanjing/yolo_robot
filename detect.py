...
FILE = Path(__file__).resolve() #detect.py 的绝对路径
ROOT = FILE.parents[0]  # YOLOv5 root directory
if str(ROOT) not in sys.path:
    sys.path.append(str(ROOT))  # add ROOT to PATH
ROOT = Path(os.path.relpath(ROOT, Path.cwd()))  # relative 转变为相对路径
...
def run(
    ...
    webcam = source.isnumeric() or source.endswith('.streams') or (is_url and not is_file)#是不是电脑摄像头
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image                #################
            ...
            save_txt_path=save_path[:len(save_path)-3]+'txt'

            txt_path = str(save_dir / 'labels' / p.stem) + ('' if dataset.mode == 'image' else f'_{frame}')  # im.txt
            LOGGER.info('save_path: %s  txt_path: %s save_txt_path %s' %(save_path,txt_path,save_txt_path

                                                                         ))
            #LOGGER.info(f"txt_path  {txt_path}"+' '+i+' '+"first")
...
##加入
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_txt:  # Write to file    original : if save_txt:
                        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                        line = (cls, *xywh, conf) if save_conf else (cls, *xywh)  # label format
                        LOGGER.info(f'{txt_path}.txt')
            ...
            with open(save_txt_path,'w',encoding='utf-8') as fs:
                fs.write(s)
            # Stream results
